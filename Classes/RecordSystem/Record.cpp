...
std::string GetShortFanText(const Record::Detail &detail) {
    ...
    if (fanFlag != 0) {
        if (__get1bitscount(fanFlag) > 1) {
            ...
            // 「一色XXX」与「混一色」或「清一色」合并「一色」二字
            size_t sameLen = strlen("一色");
            if (fanName0.compare(0, sameLen, "一色") == 0) {
                if (fanName1.compare("混一色") == 0 || fanName1.compare("清一色") == 0) {
                    return fanName1.append(fanName0.erase(0, sameLen));
                }
            }

            if (fanName1.compare(0, sameLen, "一色") == 0) {
                if (fanName0.compare("清一色") == 0) {
                    return fanName0.append(fanName1.erase(0, sameLen));
                }
            }

            // 「五门齐」与「三色XXX」可以省略「三色」二字
            size_t thereLen = strlen("三色");
            if (fanName0.compare(0, thereLen, "三色") == 0) {
                if (fanName1.compare("五门") == 0) {
                    return fanName1.append(fanName0.erase(0, thereLen));
                }
            }
