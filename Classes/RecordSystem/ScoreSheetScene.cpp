...
bool ScoreSheetScene::initWithRecord(Record *record) {
    ...
    label = Label::createWithSystemFont(__UTF8("选手姓名"), "Arial", 12);
    ...
    for (int i = 0; i < 5; ++i) {
        label = Label::createWithSystemFont(row0Text[i], "Arial", 12);
        label->setTextColor(C4B_BLACK);
        label->setPosition(Vec2(colPosX[i], line2Y));
        drawNode->addChild(label);
        cw::scaleLabelToFitWidth(label, gap - 4.0f);

        label = Label::createWithSystemFont(row1Text[i], "Arial", 12);
        label->setTextColor(C4B_BLACK);
    ...
    label = Label::createWithSystemFont(__UTF8("累计"), "Arial", 12);
    label->setTextColor(C4B_ORANGE);
    ...
    label = Label::createWithSystemFont(__UTF8("名次"), "Arial", 12);
    label->setTextColor(C4B_ORANGE);
    ...
    label = Label::createWithSystemFont(__UTF8("番种备注"), "Arial", 12);
    label->setTextColor(C4B_BLACK);
    ...
    for (int k = 0; k < 16; ++k) {
        ...
        // 东风东~北风北名字
        label = Label::createWithSystemFont(handNameText[k], "Arial", 12);
        label->setTextColor(C4B_GRAY);
        ...
        // 四位选手得分
        for (int i = 0; i < 4; ++i) {
            _scoreLabels[k][i] = Label::createWithSystemFont("", "Arial", 12);
...
#if SUPPORT_FULLY_SHOOT
    _modeLabel->setString(_record.mode == 0 ? __UTF8("传统") : __UTF8("全铳"));
#endif
...
void ScoreSheetScene::editNameAndTitle() {
...
#if SUPPORT_FULLY_SHOOT
    constexpr float limitHeight = 220.0f;
#else
    constexpr float limitHeight = 175.0f;
#endif


    Node *rootNode = Node::create();
    rootNode->setContentSize(Size(limitWidth, limitHeight));
    ...
    for (int i = 0; i < 4; ++i) {
        const float yPos = limitHeight - 15.0f - i * 25.0f;
    ...
    button1->setPosition(Vec2((limitWidth - 110.0f) / 3.0f + 27.5f, limitHeight - 120.0f));
    ...
    button2->setPosition(Vec2(limitWidth - button1->getPositionX(), limitHeight - 120.0f));
    ...
    rootNode->addChild(editBox);
    editBox->setPosition(Vec2(limitWidth * 0.5f, limitHeight - 150.0f));
    editBoxes[4] = editBox;

    Label *label = Label::createWithSystemFont(__UTF8("输入的对局名称将在标题上显示"), "Arial", 10);
    label->setTextColor(C4B_GRAY);
    rootNode->addChild(label);
    label->setPosition(Vec2(limitWidth * 0.5f, limitHeight - 170.0f));
    ...
    static const char *recordTitleText[] = { __UTF8("传统"), __UTF8("全铳") };
    for (int i = 0; i < 2; ++i) {
    ...
    AlertDialog::Builder(this)
        ...
        .setPositiveButton(__UTF8("确定"), [this, sharedEditBoxes, delegate
#if SUPPORT_FULLY_SHOOT
            , radioGroup
#endif
        ](AlertDialog *, int) {
...
void ScoreSheetScene::showSettingAlert() {
    ...
    // 1.计分表格分数显示
    Label *label = Label::createWithSystemFont(__UTF8("1. 计分表格分数显示"), "Arial", 12);
    label->setTextColor(C4B_BLACK);
    rootNode->addChild(label);
    label->setAnchorPoint(Vec2::ANCHOR_MIDDLE_LEFT);
    ...
    // 2.计分界面选手顺序
    label = Label::createWithSystemFont(__UTF8("2. 计分界面选手顺序"), "Arial", 12);
    label->setTextColor(C4B_BLACK);
    rootNode->addChild(label);
    label->setAnchorPoint(Vec2::ANCHOR_MIDDLE_LEFT);
...
void ScoreSheetScene::onResetButton(cocos2d::Ref *) {
    ...
    Label *label = Label::createWithSystemFont(__UTF8("在清空表格之前，请选择「保存」或「丢弃」"), "Arial", 12);
    rootNode->addChild(label);
...
static DrawNode *createPursuitTable(uint8_t mode, const char (&name)[4][NAME_SIZE], const int (&totalScores)[4]) {
    ...
    // n位。n=1表示二位，以此类推
    for (int n = 1; n < 4; ++n) {
        Label *label = Label::createWithSystemFont(name[indices[n]], "Arial", 12);
        ...
        for (int k = 0; k < n; ++k) {
            ...
            label = Label::createWithSystemFont(name[indices[k]], "Arial", 12);
            ...
            for (int i = 0; i < 4; ++i) {
                label = Label::createWithSystemFont(std::to_string(numbers[i]), "Arial", 12);
