...
void ExtraInfoWidget::onInstructionButton(cocos2d::Ref *) {
    ...
    Label *label = Label::createWithSystemFont(
        ...
        __UTF8("2. 本程序遵循中国国家体育总局于1998年7月审定的《中国麻将竞赛规则（试行）》，一些争议之处采用国际麻将联盟(MIL)规则委员会审定通过的《国标麻将(MCR)规则补充细则》，请以您所参加的比赛细则中之规定为准。\n")
        __UTF8("3. 手牌在形式上听多种牌（包括已经使用4张的牌）时，不计边张、嵌张、单钓将。边张、嵌张、单钓将最多计其中一个。\n")
        __UTF8("4. 组合龙基本和牌型中，可加计平和。如果听牌既可以解释为组合龙的部分，也可以解释为边张、嵌张、单钓将，可加计边张、嵌张、单钓将。\n")
        ...
        __UTF8("7. 字一色、混幺九、清幺九不计碰碰和（特殊规定）。\n")
        __UTF8("8. 至少缺一门花色的番种（包括大三元、小三元、三风刻、一色四同顺、一色四节高、一色四步高、推不倒），除推不倒不计缺一门（特殊规定）以外，其他番种均可加计缺一门。\n")
        __UTF8("9. 大四喜、小四喜可加计混一色或字一色，绿一色不计混一色（特殊规定），可加计清一色。\n")
        __UTF8("10. 清幺九不计双同刻，可加计三同刻。\n")
        __UTF8("11. 九莲宝灯和258时计1个幺九刻，和其他牌时不计幺九刻。\n")
        __UTF8("12. 不重复原则特指单个番种与其他番种的必然包含关系，不适用某几个番种同时出现时与其他番种的包含关系。例如，绿一色+清一色，必然断幺，但要计断幺。\n")
        __UTF8("13. 一明杠一暗杠计特殊番种「明暗杠」5番。双暗杠6番，双明杠4番。\n")
        __UTF8("14. 以杠字结尾的番种（四杠、三杠、双暗杠、明暗杠、双明杠、暗杠、明杠）最多同时计1个，以暗刻结尾的番种（四暗刻、三暗刻、双暗刻）最多同时计1个，杠番种与暗刻番种可同时分别计1个，但双暗杠不计双暗刻。"),
